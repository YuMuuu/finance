...
//    val withPred: Array[Row] = rows.zip(preds).zipWithIndex.map{ case ((row, pred), index) =>
//      if(index == 0) {
...
//    }
  ...
  override def write: MLWriter = GeneralMLWriter(this) // override impl
}
