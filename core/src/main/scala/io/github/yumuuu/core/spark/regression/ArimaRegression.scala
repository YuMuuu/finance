package io.github.yumuuu.core.spark.regression
...
import breeze.linalg.{DenseMatrix as BDM, DenseVector as BDV, inv as breezeInv}
import io.github.yumuuu.core.spark.regression.{ArimaParams, ArimaRegressionModel}
 ...
 */
private case class ArimaCoefficients(
                                                  phi: Array[Double],
                                                  intercept: Double)
...
class ArimaRegression(override val uid: String)
  ...
  override def fit(dataset: Dataset[?]): ArimaRegressionModel = {
...
object ArimaRegression extends DefaultParamsReadable[ArimaRegression] {
   ...
   */
  def difference(series: Array[Double], d: Int): Array[Double] = {
    if (d <= 0) {
      series
    } else {
      var result = series
